    ...
    >
      ...
      >
        <View className="flex-1">
          ...
          <Text className="text-indigo-600 text-sm font-semibold uppercase tracking-widest">X-Gaming Hub</Text>
          <Text className="text-gray-950 text-3xl font-bold mt-2">Create your account</Text>
          ...
          <TouchableOpacity activeOpacity={0.8} onPress={() => setAcceptedTerms((accepted) => !accepted)} className="flex-row items-center mt-5">
            <View className={`w-5 h-5 rounded-md border items-center justify-center mr-2 ${acceptedTerms ? "bg-indigo-600 border-indigo-600" : "bg-white border-gray-300"}`}>
              {acceptedTerms && <Ionicons name="checkmark" size={14} color="#fff" />}
            </View>
            <Text className="text-gray-500 text-sm flex-1">
              I agree to the <Text className="text-indigo-600 font-semibold">Terms & Privacy Policy</Text>
            </Text>
          </TouchableOpacity>
          ...
          <View className="flex-row justify-center items-center">
            ...
            <TouchableOpacity onPress={() => router.push("/(auth)/Login")} activeOpacity={0.7}>
              <Text className="text-indigo-600 text-sm font-bold ml-1">Sign in</Text>
            </TouchableOpacity>
          </View>
        </View>

        <Text className="text-gray-400 text-xs text-center mt-10">Your gaming hub, organized.</Text>
      </ScrollView>
    </KeyboardAvoidingView>
  );
}